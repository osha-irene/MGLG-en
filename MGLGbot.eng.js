...
if (command === "!realm") {
    if (!args[0]) {
        return message.reply("❌ Please specify a realm. Example: `!realm Planet`");
    }

    const realm = args[0].trim();
    const validRealms = ["Planet", "Animalism", "Dynamics", "Poetics", "Visions", "Shadows"];

    if (!validRealms.includes(realm)) {
        return message.reply(`❌ Invalid realm. Choose from: (${validRealms.join(", ")})`);
    }
    ...
    message.reply(`✅ Your realm has been set to **${realm}**.`);
}
...
if (command === "!check_sheet") {
    ...
    const magicalNameOutput = char.magicalName || "Unset"; // ✅ Magical Name 추가
...
// Ensure "Calling" exists in the book list
if (!char.books["Calling"]) {
    char.books["Calling"] = {
...
const bookList = Object.keys(char.books).length > 0
    ? Object.entries(char.books)
        .map(([name, { type, targetStamp, vitaRealm, vitaCost, currentVita }]) => {
            const emoji = vitaRealm && realmEmojis[vitaRealm] ? realmEmojis[vitaRealm] : "📖";
            currentVita = currentVita ?? 0;
            const vitaDisplay = vitaCost && vitaCost > 0 ? `${currentVita} / ${char.stats?.source || courseOutput}` : "None";
            return `${emoji} **${name}** | ${type || "None"} | ${targetStamp || "None"} | ${vitaDisplay}`;
        })
        .join("\n")
    : "None";
    ...
    const realmOutput = char.realm || "Unset"; // ✅ Realm 추가

    // 🔮 Display Ether
    const etherOutput = char.ether ? `Ether ${char.ether} | ` : "Ether 0 |";
    ...
    // 🔹 Display Stats (Attack, Defense, Source)
    const statsOutput = char.stats
        ? `Attack ${char.stats.attack || courseOutput} | Defense ${char.stats.defense || courseOutput} | Source ${char.stats.source || courseOutput}`
        : `Attack ${courseOutput} | Defense ${courseOutput} | Source ${courseOutput}`;

    // 📜 **Final Output (Formatted with block quotes)**
    message.reply(
        `📖 **Character Information**
> **${courseOutput}th Course ${instituteOutput} - ${hierarchyOutput}**
> 🔹 Magician of ${realmOutput} **「${magicalNameOutput}」**
> 🔹 ${characterName}
...
if (command === '!help') {
    const helpMessages = [
            ...
            fields: [
                {
                    ...
                    value: '> `!register_character "Name" Stamp1 Stamp2 Stamp3 Stamp4 Stamp5` - Register character\n' +
                           '> `!set "Character Name"` / `!unset` - Activate/deactivate character\n' +
                           '> `!check_sheet` - View current character info\n' +
                           '> `!course [1~7]` - Set Course (Adjusts stats)\n' +
                           '> `!institute [Table of Contents/Horizon/.../Laboratory]` - Set Institute\n' +
                           '> `!hierarchy [Book Watcher/Librarian/.../Apocrypha]` - Set Hierarchy\n\n' +
                           '**🎭 Stamp of Spirit**\n' +
                           '> `!set_ss [Stamp Name]` - Set Stamp of Spirit (Fixed target value 6)\n' +
                           '> `!check_ss` - Check set Stamp of Spirit\n'
                }
            ]
        },
        {
            ...
            fields: [
                ...
                {
                    name: '**📌 Plot System**',
                    value: '> `/plot [1~6]...` - Set plot (Up to 6)\n' +
                           '> `!randomplot [Count]` - Generate random plot\n' +
                           '> `!revealplot` - Reveal stored plots\n'
                },
                {
                    name: '**📌 Book Management**',
                    value: '> `!addbook "Book Name" Type Stamp Vita Realm Vita Cost Description` - Add a book\n' +
                           '> `!cast "Book Name"` - Use a book for a roll\n' +
        ...
        },
        {
            ...
            fields: [
                ...
                {
                    name: '📌 Support',
                    value: 'For inquiries and feedback, contact Osha (@TRPG_sha/o3o_sha).'
                }
            ]
        }
    ];
    ...
    // 🛠 Embed 메시지를 순차적으로 전송
    helpMessages.forEach(async msg => {
